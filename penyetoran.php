    ...
    <div class="card shadow-lg p-3 mb-5">
        <div class="card-body">
            <form action="" method="POST" enctype="multipart/form-data">
                ...
                <br>
                <button type="submit" name="simpan" class="btn btn-success" onclick="return confirm('anda yakin ingin melakukan penyetoran?')">setor</button>
            </form>
        </div>
    </div>
</div>
<?php
    if (isset($_POST['simpan'])) {
        ...
        $query = mysqli_query($koneksi,"INSERT INTO tb_transaksi VALUES(null,'$nis','$nominal',0,'$waktu')");
        mysqli_query($koneksi,"UPDATE tb_siswa SET saldo=saldo+$nominal WHERE NIS = '$nis'");
        if ($query) {
            echo "<script>alert('penyetoran berhasil')</script>";
            echo "<script>location='index.php?hal=transaksi'</script>";
        }else{
            echo "<script>alert('penyetoran gagal')</script>";
            echo "<script>location='index.php?hal=penyetoran'</script>";
        }
    }
?>